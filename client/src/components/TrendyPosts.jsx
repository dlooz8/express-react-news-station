...
function TrendyPosts() {
    ...
    const currentPosts = trendyPosts.slice(indexOfFirstPost, indexOfLastPost);
    
    const nextPage = () => {
        if (indexOfLastPost < trendyPosts.length) {
    ...
    <section className="container mx-auto">
        <div className="flex justify-between py-6">
            ...
            <div className="flex justify-between gap-4">
                <button onClick={prevPage} disabled={currentPage === 0} className="w-[40px] h-[40px] bg-gray rounded-xl flex items-center justify-center red-hover">
                ...
                <button onClick={nextPage} disabled={indexOfLastPost >= trendyPosts.length} className="w-[40px] h-[40px] bg-gray rounded-xl flex items-center justify-center red-hover">
                    <svg width="24" height="24" viewBox="0 0 24 24" fill="#3E3232" xmlns="http://www.w3.org/2000/svg">
        ...
        <div className="flex justify-between gap-3">
            ...
            <motion.div className="flex flex-col justify-between p-3 shadow rounded-xl items-center w-[270px] 2xl:w-[360px] h-[340px] 2xl:h-[390px]"
                key={index}
                initial={{ opacity: animate ? 0 : 1, x: animate ? (leftAnimate ? -30 : 30) : 0 }}
                animate={{ opacity: animate ? 1 : 0, x: animate ? 0 : (leftAnimate ? 30 : -30) }}
                exit={{ opacity: 0, x: 30 }}
                transition={{ duration: 0.3 }}>
                <Link to={`/news/${post.post_id}`}className="flex flex-col gap-2">
                ...
                <div className="flex justify-between items-center gap-4 bg-gray rounded-xl p-3 w-full">
                    ...
                    <div className="red-hover pr-2" onClick={() => AddBookmark(post.post_id, isUser.id)}>
